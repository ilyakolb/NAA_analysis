classdef NAA_curation < Singleton
    ...
    methods
        ...
        function createSingletons(obj) %#ok<MANU>
            ...
            Protocol('GCaMP96', 'GCaMP', [1, 3, 10, 160], [0.3, 0.7, 1.5, 7], [1, 1, 1, 1; 1, 1, 1, 1] / 1000, [8, 12], Construct('10.560'));
            Protocol('FRET', 'FRET');
            ...
            Protocol('RCaMP96b', 'RCaMP', [1, 3, 10, 160], [0.3, 0.7, 1.5, 7], [1, 1, 1, 1; 1, 1, 1, 1], [8, 12], Construct('106.1'));
        ...
        function populatePlateSets(obj)
            if ispc
                ...
                for i = double('z'):-1:double('c')
                    if exist(['' i ':\GENIE Pipeline'], 'dir')
                        obj.pipelinePaths.primary = [i '\GENIE Pipeline'];
                    end
                    if exist(['' i ':\GECI Pipeline'], 'dir')
                        obj.pipelinePaths.archive = [i '\GECI Pipeline'];
                    end
                end
            elseif ismac
                % Determine the path to the primary and archive shares using the command line 'mount' utility.
%                 [status, obj.pipelinePaths.primary] = system('mount | grep -i dm11/GENIE | sed "s/.* on \(.*\) (.*/\1/g"');
                  [status, obj.pipelinePaths.primary] = system('mount | grep -i nearline/GENIE | sed "s/.* on \(.*\) (.*/\1/g"');
                if status == 0 && ~isempty(obj.pipelinePaths.primary)
                    obj.pipelinePaths.primary = fullfile(obj.pipelinePaths.primary(1:end-1), 'GENIE Pipeline');
                end
                ...
                if status == 0 && ~isempty(obj.pipelinePaths.archive)
                    obj.pipelinePaths.archive = fullfile(obj.pipelinePaths.archive(1:end-1), 'GECI Pipeline');
                end
            else
                obj.pipelinePaths.primary = '/groups/flylight/GENIE/GENIE Pipeline';
                obj.pipelinePaths.archive = '/archive/genie/GECI Pipeline';
            end
            
            imagingPaths.primary = fullfile(obj.pipelinePaths.primary, 'Imaging Data');
            imagingPaths.archive = fullfile(obj.pipelinePaths.archive, 'Imaging Data');
...
%         end
        ...
        function createDataAllFile(obj, ~, ~)
                ...
                if controlCount == 0
                    ...
                    if adequateControls
                        ...
                        if strcmp(passedWells(1,1).parent.protocol.name,'RCaMP96b')||strcmp(passedWells(1,1).parent.protocol.name,'GCaMP96')
                            ...
                            [controldprime controlSNR] = control.dprime(fm);
                            clear fm
                        end
                    ...
                    if strcmp(passedWells(1,1).parent.protocol.name,'RCaMP96b')||strcmp(passedWells(1,1).parent.protocol.name,'GCaMP96') %modified Hod 20140119
                        fprintf(fid, 'dprime\t');
                        headerRow.createCell(colNum).setCellValue('d-Prime'); colNum = colNum + 1; %added by Hod 20131123 - dprime and SNR data
                        for j = 1:numPulses
                    ...
                    for i = 1:length(constructs)
                        ...
                        if numReplicates > 0
                            ...
                            if strcmp(passedWells(1,1).parent.protocol.name,'RCaMP96b')||strcmp(passedWells(1,1).parent.protocol.name,'GCaMP96') %modified Hod 20140119
                                ...
                                [dprime, SNR] = construct.dprime(fm);
                                fprintf(fid, '%f\t', dprime);
                                wbCell = wbRow.createCell(colNum); colNum = colNum + 1;
                                wbCell.setCellValue(dprime);
                                for j = 1:numPulses
                            ...
                            if createPileAllUpTo
                                ...
                                if strcmp(passedWells(1,1).parent.protocol.name,'RCaMP96b')||strcmp(passedWells(1,1).parent.protocol.name,'GCaMP96') %modified Hod 20140119
                                    [dprime SNR]=construct.dprime(mutantPile(end).fmean);
            ...
            if isempty(errorMessage)
                resultsDir = fullfile(obj.pipelinePaths.primary, 'Analysis', 'Results');
