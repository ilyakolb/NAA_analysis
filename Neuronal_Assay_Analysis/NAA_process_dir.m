...
if length(files) > 3 && (strncmp(plate_folder_name, 'Well', 4) || strncmpi(plate_folder_name, '96Well', 6))
    %data folder
    cd(plate_folder_path);
    disp(['Switching to ' , plate_folder_path,' for processing'])
    
    if exist('Segmentation.mat', 'file') || exist('segmentation_cherry.mat', 'file')
        disp('This directory has already been processed.');
    else
        if strcmpi(type, 'GCaMP96') || strcmpi(type, 'RCaMP96') || strcmpi(type, 'FRET96') || strcmpi(type, 'RCaMP96b') || strcmpi(type, 'OGB1')
            segment_file_ID = 4;
        ...
        try
            NAA_script(segment_file_ID, nominal_pulse, type, segmentation_threshold);
        catch ME
    ...
    for i=3:length(subfolders) 
        NAA_process_dir(fullfile(plate_folder_path, subfolders(i).name), type, segmentation_threshold);
    end
end
