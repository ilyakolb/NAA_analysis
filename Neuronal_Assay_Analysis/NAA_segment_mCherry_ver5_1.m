function ROI_list = NAA_segment_mCherry_ver5_1(GCaMPbase, mCherry, dF, type, segmentation_threshold,GCaMPbase2)
...
global DEBUG; DEBUG=1; %% IK MOD change back to DEBUG = 0;
...
elseif(strcmpi(type, 'RCaMP96b')||strcmpi(type, 'RCaMP96c'))
    ...
    filt_map_order=sort(reshape(filt_map,1,[]));
    th=filt_map_order(round(0.992*numel(filt_map_order)));
elseif(strcmpi(type, 'GCaMP96bf'))||strcmpi(type, 'GCaMP96uf')||strcmpi(type, 'RCaMP96uf')
    filt_map_order=sort(reshape(filt_map,1,[]));
...
elseif strcmpi(type, 'RCaMP96')||strcmpi(type, 'FRET')||strcmpi(type, 'OGB1')
    ...
    radius_neu=4; % IKMOD radius_neu=4;
end
...
if ~isempty(segmentation_threshold)
    ...
    if segmentation_threshold==0
        ...
        elseif strcmp(type, 'GCaMP96bf')||strcmpi(type, 'GCaMP96uf')||strcmpi(type, 'RCaMP96uf')%updated 20170719
        ...
        elseif strcmp(type, 'GCaMP96bf') || strcmp(type, 'GCaMP96c')||strcmpi(type, 'GCaMP96uf')||strcmpi(type, 'RCaMP96uf')%added 20170306, updated 20170719
            noise_med_estim=smc(round(0.6*length(smc)));
...
if (strcmpi(type, 'RCaMP96b')||strcmpi(type, 'GCaMP96')||strcmp(type, 'GCaMP96b')||strcmpi(type, 'RCaMP96c'))||...
        ...
        ||strcmpi(type, 'GCaMP96uf')||strcmpi(type, 'RCaMP96uf')
    disc2=th_nucleus;
end
v2=[ROI_list.mCherry];
if (strcmpi(type, 'RCaMP96b')||strcmpi(type, 'GCaMP96')||strcmp(type, 'GCaMP96b')||strcmpi(type, 'RCaMP96c'))||...
        strcmp(type, 'GCaMP96bf')||strcmp(type, 'GCaMP96c')||strcmpi(type, 'GCaMP96uf')||strcmpi(type, 'RCaMP96uf')  
...
for k=1:length(ROI_list)
    if high_expression(k)
        if roundness(k)>0.5 && area(k)>20 % IK relaxed constraints roundness(k)>0.73&&area(k)>20
...
%         end?
        ...
        if (strcmp(type,'GCaMP96bf'))||strcmpi(type, 'GCaMP96uf')||strcmpi(type, 'RCaMP96uf')
            t2 = kmean1D_threshold(v2,0.5);
        end
