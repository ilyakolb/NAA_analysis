...
% subplot(2,2,3);
% NAA_displayROI(cell_list);
% subplot(2,2,4);
% NAA_displayROI(cell_list,df_fmap(:,:,5));
figure;
subplot(2,2,1);
imagesc(df_fmap(:,:,1));axis image;
subplot(2,2,2);
imagesc(df_fmap(:,:,3));axis image;
subplot(2,2,3);
imagesc(df_fmap(:,:,5));axis image;
subplot(2,2,4);
imagesc(df_fmap(:,:,9));axis image;


% figure;
% NAA_displayROI(cell_list,df_fmap(:,:,5));
figure;
NAA_displayROI(cell_list,df_fmap(:,:,end));
figure;
NAA_displayROI(cell_list,overlay)
figure;
NAA_displayROI(cell_list)
...
nAP=[ephus_info.nIPulse];
nAP_Pts=length(nAP);
nTrial=size(para_array,1)/nAP_Pts;
fs=35;   %Hz
nTime=length(para_array(1).df_f);
nROI=size(para_array,2);
