function NAA_display_Plate_r2016b(type)
% Create the PDF
if strcmp(type,'GCaMP96b-ERtag')
    type='GCaMP96b';
end
if strcmp(type, 'GCaMP96') ||strcmpi(type, 'GCaMP96b') || strcmpi(type, 'RCaMP96')|| strcmpi(type, 'RCaMP96b')||strcmpi(type, 'FRET96') ||strcmpi(type, 'OGB1')||...
        strcmpi(type, 'RCaMP96c')||strcmpi(type, 'GCaMP96z')||strcmpi(type, 'RCaMP96z')||strcmpi(type, 'GCaMP96bf')||strcmpi(type, 'GCaMP96c')||(strcmpi(type, 'GCaMP96d'))...
        ||strcmpi(type, 'GCaMP96u')||strcmpi(type, 'RCaMP96u')||strcmpi(type, 'GCaMP96uf')|| strcmpi(type, 'mngGECO')||strcmpi(type, 'RCaMP96uf') %OGB added by Hod 20131125 %% FRET96 was added by Hod 01Apr2013, GCaMP96z on 20160919
    row = {'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'};
...
for i=1:ntotalwell
    ...
    if ~isempty(file)
        ...
        elseif strcmpi(type, 'GCaMP96bf')||strcmpi(type, 'GCaMP96uf')|| strcmpi(type, 'mngGECO')||strcmpi(type, 'RCaMP96uf')
            S=load([file(1).name],'GCaMPbase','GCaMPbase2','mCherry','cell_list','summary','df_fmap');
        else
        ...
        if strcmpi(type, 'GCaMP96') ||strcmpi(type, 'GCaMP96b') || strcmpi(type, 'RCaMP96')||strcmpi(type, 'RCaMP96b')||...
                strcmpi(type, 'FRET96') ||strcmpi(type, 'OGB1')|| strcmpi(type, 'RCaMP96c')||strcmpi(type, 'GCaMP96z')||...
                strcmpi(type, 'RCaMP96z')||strcmpi(type, 'GCaMP96bf')||strcmpi(type, 'GCaMP96c')||(strcmpi(type, 'GCaMP96d'))...
                ||strcmpi(type, 'GCaMP96u')||strcmpi(type, 'RCaMP96u')||strcmpi(type, 'GCaMP96uf')|| strcmpi(type, 'mngGECO')||strcmpi(type, 'RCaMP96uf')%updated 20170719
...
if exist(pdf_name, 'file')
    disp('Results PDF file already exists. Overwriting');
end
...
for i = 1:ntotalwell
    ...
    if isempty(mr)
        ...
        if isempty(file)
            ...
            %                     else
            if strcmp(type, 'GCaMP96bf')||strcmpi(type, 'GCaMP96uf')|| strcmpi(type, 'mngGECO')||strcmpi(type, 'RCaMP96uf') %modified by Hod 20170309
                S = load(fullfile('..', well_dir(1).name, segment_name), base_name, 'GCaMPbase2', 'mCherry');
                overlay = NAA_create_overlay(S.GCaMPbase2, S.mCherry);
            else
    ...
    elseif strcmpi(type, 'RCaMP96') || strcmpi(type, 'RCaMP96b')|| strcmpi(type, 'RCaMP96c')||strcmpi(type, 'RCaMP96u')
        overlay = NAA_create_overlay(mr.mCherry, mr.GCaMPbase);
    elseif strcmpi(type, 'GCaMP96bf')||strcmpi(type, 'GCaMP96uf')|| strcmpi(type, 'mngGECO')||strcmpi(type, 'RCaMP96uf')
        overlay = NAA_create_overlay(mr.GCaMPbase2, mr.mCherry);
    else
    ...
    if isempty(mr)
        ...
        line([radius - offset, radius + offset], [radius + offset, radius - offset], ...
            'Color', [0.5 0 0], ...
            'LineWidth', 8);
    elseif strcmpi(type, 'GCaMP96') ||strcmpi(type, 'GCaMP96b') || strcmpi(type, 'RCaMP96') || strcmpi(type, 'RCaMP96b') ||...
            strcmpi(type, 'OGB1')|| strcmpi(type, 'RCaMP96c')||strcmpi(type, 'GCaMP96z')||strcmpi(type, 'RCaMP96z')||...
            strcmpi(type, 'GCaMP96bf')||strcmpi(type, 'GCaMP96c')||(strcmpi(type, 'GCaMP96d'))||strcmpi(type, 'GCaMP96u')||strcmpi(type, 'RCaMP96u')...
            ||strcmpi(type, 'GCaMP96uf')|| strcmpi(type, 'mngGECO')||strcmpi(type, 'RCaMP96uf')
...
for k=1:length(nAP)
    ...
    % IK fixed to include AP numbers in file name
    print([plate_name '_' num2str(nAP(k)) 'AP.pdf'],'-dpdf','-fillpage')
    clf(fig_h);
end
