function ROI_list = NAA_segment_mCherry_ver5(GCaMPbase, mCherry, dF, type, segmentation_threshold)
...
elseif(strcmpi(type, 'GCaMP96b')) %added by Hod 20140811, updated 20160916 HD
    filt_map_order=sort(reshape(filt_map,1,[]));
    th=filt_map_order(round(0.95*numel(filt_map_order)));
elseif(strcmpi(type, 'GCaMP96z'))||(strcmpi(type, 'RCaMP96z'))
    filt_map_order=sort(reshape(filt_map,1,[]));
    th=filt_map_order(round(0.98*numel(filt_map_order)));
...
if ~isempty(segmentation_threshold)
    ...
    if segmentation_threshold==0
        ...
        elseif strcmp(type, 'GCaMP96z')||(strcmpi(type, 'RCaMP96z')) %added by Hod 20160916, updated 20161122
...
disc = th;
if (strcmpi(type, 'RCaMP96b')||strcmpi(type, 'GCaMP96')||strcmp(type, 'GCaMP96b')||strcmpi(type, 'RCaMP96c'))||strcmp(type, 'GCaMP96z')||(strcmpi(type, 'RCaMP96z'))
    disc2=th_nucleus;
end
v2=[ROI_list.mCherry];
...
if ~isempty(ROI_list)
    ...
    if strcmp(type, 'FRET')
        ...
        %data
        if ~(strcmp(type,'GCaMP96b')||strcmp(type,'GCaMP96b'))
