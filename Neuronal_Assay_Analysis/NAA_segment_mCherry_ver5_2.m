function ROI_list = NAA_segment_mCherry_ver5_2(GCaMPbase, mCherry, dF, type, segmentation_threshold)
...
filt_map=imfilter(mCherry,h2,'symmetric');
...
elseif(strcmpi(type, 'GCaMP96b')) %added by Hod 20140811, 
    filt_map_order=sort(reshape(filt_map,1,[]));
    th=filt_map_order(round(0.95*numel(filt_map_order)));
elseif(strcmpi(type, 'GCaMP96z'))||(strcmpi(type, 'RCaMP96z'))||(strcmpi(type, 'GCaMP96c'))||(strcmpi(type, 'GCaMP96d'))||strcmpi(type, 'RCaMP96u')||strcmpi(type, 'GCaMP96u') %updated 20160916 anf 20170314 20170417 HD
    filt_map_order=sort(reshape(filt_map,1,[]));
    th=filt_map_order(round(0.98*numel(filt_map_order)));
...
for i=1:length(col)
    ...
    if length(pixel_list)>length(nuc_list)
        ...
        if strcmpi(type, 'GCaMP96bf')
            convert_cell_coordinates_GCaMPbf;
...
if ~isempty(segmentation_threshold)
    ...
    if segmentation_threshold==0
        ...
        elseif strcmp(type, 'GCaMP96c')||(strcmpi(type, 'GCaMP96d'))||strcmpi(type, 'RCaMP96u')||strcmpi(type, 'GCaMP96u') % updated 20170314, 20170719
        ...
        elseif strcmp(type, 'GCaMP96bf')%updated 20170306
            gc=reshape(GCaMPbase2,[],1);
            sgc=sort(gc);
            noise_med_estim=sgc(round(0.5*length(sgc)));
            noise_std_estim=std(sgc(round(0.2*length(sgc)):round(0.65*length(sgc))));
            th=noise_med_estim+6*noise_std_estim; %new expression threshold for GCaMP96z, should catch dim cells
        end
        %         if strcmp(type, 'GCaMP96')
        ...
        elseif strcmp(type, 'GCaMP96bf') || strcmp(type, 'GCaMP96c')||(strcmpi(type, 'GCaMP96d'))||strcmpi(type, 'RCaMP96u')||strcmpi(type, 'GCaMP96u')%added 20170306, updated 20170314, 20170719
            noise_med_estim=smc(round(0.6*length(smc)));
...
if (strcmpi(type, 'RCaMP96b')||strcmpi(type, 'GCaMP96')||strcmp(type, 'GCaMP96b')||strcmpi(type, 'RCaMP96c'))||...
        strcmp(type, 'GCaMP96z')||(strcmpi(type, 'RCaMP96z'))||strcmp(type, 'GCaMP96bf')||strcmp(type, 'GCaMP96c')||(strcmpi(type, 'GCaMP96d'))...
        ||strcmpi(type, 'RCaMP96u')||strcmpi(type, 'GCaMP96u')
    disc2=th_nucleus;
end
v2=[ROI_list.mCherry];
if (strcmpi(type, 'RCaMP96b')||strcmpi(type, 'GCaMP96')||strcmp(type, 'GCaMP96b')||strcmpi(type, 'RCaMP96c'))||...
        strcmp(type, 'GCaMP96bf')||strcmp(type, 'GCaMP96c')||(strcmpi(type, 'GCaMP96d'))||strcmpi(type, 'RCaMP96u')||strcmpi(type, 'GCaMP96u')
...
expression_th=median(expression_level)+0.8*(max(expression_level)-median(expression_level));
high_expression=expression_level>expression_th;
for k=1:length(ROI_list)
    if high_expression(k)
        if roundness(k)>0.73&&area(k)>20
            remove_ind(k)=0;
        else
            remove_ind(k)=1;
        end
